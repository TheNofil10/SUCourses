...
stmt: FuncDefinition
    | declaration{searchdecl(globalvars,$1,lineno);
push(globalvars,$1 );//search if it exists before pushing
}
    | assignment{searchundef(globalvars,$1,lineno);}
    | printstmt
;
...
bodystmt: declaration{
	searchdecl(globalvars,$1,lineno);
	searchdecl(localvars,$1, lineno);
	push(localvars,$1);
	
}
	| assignment{
	int check1 = searchundef(globalvars,$1,lineno);
    int check2 = searchundef(localvars,$1,lineno);
...
formalParameter: type tIDENT{searchdecl(globalvars,$2,lineno);
	       			push(localvars,$2);}
;
...
value: tINTVAL{} | tSTRINGVAL{} | tIDENT{int check1 = searchundef(globalvars,$1,lineno);
    int check2 = searchundef(localvars,$1,lineno);
    int check3 = check1+ check2;
    if(check3 ==1)
    {;
